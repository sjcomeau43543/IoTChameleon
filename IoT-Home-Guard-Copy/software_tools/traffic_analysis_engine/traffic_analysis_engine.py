...
class TrafficAnalysisEngine(object):
	...
	def UDP_analyze(self,packet): # TODO: check ports
		if (packet.source == self.rules['device'][0]) and (packet.destination == self.rules['packet'][1]['dst_IP']): #and (packet.info.split(' ')[0] == self.rules['packet'][1]['src_port']) and (packet.info.split(' ')[2] == self.rules['packet'][1]['dst_port']):
			return True
		# elif (packet.source == self.rules['packet'][1]['IP']): #and packet.info.split(' ')[0] == self.rules['packet'][2]['port']):#or (packet.destination == self.rules['packet'][2]['IP'] and packet.info.split(' ')[2] == self.rules['packet'][2]['port']):
		# 	return True
		else:
			return False

	def SSDP_analyze(self,packet): # TODO: check ports
		for domain_IP in self.domain_ip[0]:
			if (packet.source == self.rules['device'][0]) and (packet.destination == domain_IP):
				return True
			# if (packet.source == self.rules['packet'][3]['IP']):
			# 	return True
		return False
	...
	def traffic_analyze(self,packet):
		...
		if prot == "SSDP":
			return self.SSDP_analyze(packet)
		# elif prot == "ICMP":
